...
class RidgeRegression:
    def __init__(self, learning_rate=0.1, epochs=1000, rlambda=10) -> None:
    ...
    def fit(self, X, actualY):
        ...
        for epoch in range(self.epochs + 1):
            ...
            mse_term = (1 / (2 * numberofExamples)) * np.sum(error**2)
            penalty_term = (self.rlambda / (2 * numberofExamples)) * np.sum(
                self.weights[:-1] ** 2
            )
            ...
            regularizedGradient = unregularizedGradient.copy()
            regularizedGradient[:-1] += (
                self.rlambda / numberofExamples
            ) * self.weights[:-1]

            self.weights = self.weights - self.learning_rate * regularizedGradient
...
# --- Train ---
model = RidgeRegression(learning_rate=1.0, epochs=10000)
...
# --- Sklearn Comparison ---
sk = SklearnRidge(alpha=model.rlambda)
